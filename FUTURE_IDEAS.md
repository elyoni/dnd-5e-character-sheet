...
Raised during the usability-plan interview (see `USABILITY_PLAN.md`) as item 9, then deliberately pulled out to build separately later. Design reached during that interview, kept here so it doesn't need to be re-derived:

- Each beast form is a **full separate character**, reusing the existing `charType:"animal"` machinery — not an in-place stat-swap on the Druid's own `state`. Considered and rejected: snapshotting/restoring the Druid's own AC/HP/speed/attacks in place, and a lighter-weight "uses-tracker + notes" version that stops short of real stats. Both were rejected in favor of the full-stat-swap read of the request once it became clear the app's character-switching infrastructure already does most of the work.
- Add a `linkedTo` field (beast character → owning PC's id) to `defaultChar`/`hydrate`. Purely local grouping metadata for the character switcher (shows linked beasts nested/indented under their Druid) — explicitly **not** a general-purpose character-grouping/folder system, and has no effect on export/share, which stays fully per-character exactly as today (confirmed this doesn't interact with the existing `#char=...` share-link mechanism).
- Beast forms are entirely **user-defined** (name, abilities, AC, HP, speed, attacks) — no built-in monster database to build or maintain.
- "Transform" / "Transform back" = switching the active character between the Druid and a linked beast via the existing character switcher. **No auto-revert at 0 HP** — manual switch only, since HP is a plain editable number everywhere else in the app with no combat engine watching it; adding a special case just for this would be inconsistent.
