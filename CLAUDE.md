...
- `src/data/races-classes.js` — a **generated** plain-const data file (`const RACES = {...}; const CLASSES = {...}; const BACKGROUNDS = {...};`), compiled by `scripts/compile-content.js` from the hand-edited markdown vault `content/races/*.md` / `content/classes/*.md` / `content/backgrounds/*.md` (YAML frontmatter — traits, resources, saves, hit die, skill suggestions, kids/adult blurbs, counter-stereotype "combos" — plus an optional markdown body for lore). The filename predates `BACKGROUNDS`; kept as-is rather than churning every marker/path that references it. **Edit the `content/**/*.md` files**, not this generated file. `node build.js` runs `compileContent()` first, so it always regenerates before building either HTML page; running `scripts/compile-content.js` on its own also works for a quick recompile. It's injected into both HTML outputs via the same `<!-- BUILD:MODULE ... -->` marker mechanism used for `dice-physics.js`. Mechanical fields (traits/resources/saves/skill+tool proficiencies/Origin feat/etc.) are read by `dnd_character_sheet.src.html`'s `applyRaceTemplate()`/`applyTemplate()`/`applyBackgroundTemplate()`; narrative fields (blurbs/combos) are Field-Guide-only. `compileContent()` validates the vault (missing required sub-fields, a `combos[].cls`/`combos[].race` that isn't a known id, etc.) and refuses to write anything if it finds a problem, printing every issue found rather than stopping at the first. Backgrounds are a 2024-rules third axis independent of Race — their `combos[]` pair with a class, same shape as a race's, never with a race.

There is a second, smaller page built the same way: `field-guide.src.html` → `field-guide.html` (also git-ignored), a kid-friendly, explorable races/classes/backgrounds browser styled as a little illustrated book (cover → contents → flip-card roster → a per-entry "spotlight" page with a filmstrip you page through) built specifically to avoid the fantasy-art stereotype of pairing certain races with certain classes (an Elf Barbarian gets equal billing to an Orc Barbarian). It reads the same `RACES`/`CLASSES`/`BACKGROUNDS` data via the same marker — including `label:{en,he}`, which all three content types carry (classes only gained it once the Field Guide needed a Hebrew class name; keep it in sync if a class/background is renamed). It connects to the main sheet at exactly one point each way: the sheet's New Character modal links out to it ("Not sure yet? Browse the Field Guide"), and picking a race/class/background/combo there hands back a prefilled character via the same `#char=...` share-link hash the sheet's own Export already produces (`buildPickedCharacterState()` in `field-guide.src.html` mirrors `defaultChar()`'s shape, since the receiving side just runs `hydrate()` on whatever it's given). `docs/adr/0008-field-guide-shares-data-with-sheet.md` has the full rationale.

Run `node build.js` locally after editing `dnd_character_sheet.src.html`, `field-guide.src.html`, `src/dice-physics.js`, or any `content/**/*.md` file — it's how you get a `dnd_character_sheet.html`/`field-guide.html` to open/test at all, since neither is committed. `package.json` pulls in Playwright for end-to-end tests and `gray-matter` for frontmatter parsing in `compile-content.js` (there's still no bundler/transpiler involved in the HTML build — `build.js` is dependency-free string concatenation once the content vault is compiled).
...
| Class/race/background template application (`applyTemplate`, `applyRaceTemplate`, `applyBackgroundTemplate`) — all three read from `CLASSES`/`RACES`/`BACKGROUNDS` (compiled from `content/**/*.md`), not hardcoded tables | 3452–3623 |
...
- New race/class/background "quick add" content goes in the matching `content/**/*.md` file (traits/resources/saves/skills/tool/feat fields — see `scripts/compile-content.js`'s `validate()` for the exact shape each field needs), consumed by `applyRaceTemplate()` / `applyTemplate()` / `applyBackgroundTemplate()` — these only *add* to existing state, never overwrite what a user already filled in.
...
- **Keep `README.md` in sync.** It documents user-facing content — races/subraces and their granted traits, classes and their granted saves/skills/resources/spellcasting, backgrounds and their granted skills/tool/feat, the feat system, and other player-visible features/panels. Any change that adds/removes/modifies one of these (e.g. a new entry in `RACE_LIST`/`SUBRACE_MAP`/`CLASS_LIST`/`BACKGROUND_LIST`, a tweak to `applyRaceTemplate()`/`applyTemplate()`/`applyBackgroundTemplate()`, a new panel or mechanic) must update the corresponding section of `README.md` in the same change.
- **Keep the AI-assisted flows in sync with the field shapes they describe.** `AI_ADDITION_SCHEMAS` (feats/traits/attacks/spells/resources/items defaults) and the YAML schema examples baked into `buildAIExport()` ("Ask AI") and `buildCreateAIExport()` ("Create with AI") are hand-maintained prose/YAML, not derived from the real row shapes — the AI only knows about a field if it's spelled out there. Any new or renamed field on an addable entry (a new item property, a new attack/spell property, a new top-level proficiency, etc.) needs the matching key added to `AI_ADDITION_SCHEMAS[cat].defaults` *and* to both prompt-builder functions' example block and rules text, or AI-suggested additions will silently omit it with no error.
