#!/usr/bin/env node
// Concatenation build: reads dnd_character_sheet.src.html (the editable
// source template) plus the vendored libraries and the dice-physics module,
// and writes dnd_character_sheet.html — the single self-contained file that
// actually ships (deploy.yml copies it as-is, unchanged from before this
// build step existed). No bundler, no transpilation — just string
...
function build() {
  if (!fs.existsSync(SRC_HTML)) {
    throw new Error(`${SRC_HTML} not found — this build reads the .src.html template, not dnd_character_sheet.html directly.`);
  }
  ...
  const banner = `<!-- GENERATED FILE — do not edit directly. Edit dnd_character_sheet.src.html and/or src/dice-physics.js, then run \`node build.js\`. -->\n`;
  html = banner + html;
