...
- **Spellcasting** — spell slots (levels 1–9) with click-to-toggle used/available dots, a spell list, and dice-rolled spell damage. The New Spell dialog includes an optional premade-spell picker for classes with a defined preset list (currently Cleric, Druid, and Ranger — a handful of common cantrips/1st/2nd-level spells each) that pre-fills level/name/damage/notes, which you can still edit before adding; other classes (and a blank or custom class) simply don't show the picker, and manual entry always works.
