...
**Traits** are things your race or class *gives* you — Darkvision, Fey Ancestry, Dwarven Toughness — never a player choice. **Feats** are the player's own *optional* picks, usually taken instead of an Ability Score Improvement (e.g. Alert, Tough). The sheet keeps them in two separate panels (Traits, then Feats) so a glance at either tells you where it came from, but mechanically they're identical: both use the exact same modifier system below, and every derived-stat calculation reads both lists together — a numeric effect works the same whichever panel it sits in.

Racial traits are added to the **Traits** panel automatically by the race template (see [Races](#races) above); feats are added by hand (or via Ask AI) to the **Feats** panel. Both panels use the same "add" modal and the same generic modifier:
