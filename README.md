...
- **Import / export** — download/upload a character as JSON, or share one via a self-contained link (character data is gzip-compressed, then base64-encoded, into the URL hash — a browser without compression support falls back to an uncompressed link, and either format decodes correctly regardless of which browser generated it or is opening it). Newer app versions always stay able to import files exported by older versions, links included. Import (⬆) is a small modal offering either a file upload or a box to paste a share link into directly — useful when a link arrives as plain text rather than something tappable.
